...
const AddInfomation = ({ setNewLookData, newLookImg, setCurrentStep }) => {
  ...
  return (
    <AddInformationWrapper>
      ...
      <CustomDivider className="content-divider" vertical color="#fff" />
      <FormWrapper form={form} onFinish={onFinish}>
        ...
        <Form.Item
          name="name"
          label={
            <div className="text-uppercase">{t("input.name.placeholder")}</div>
          }
        >
          <Input />
        </Form.Item>
          ...
          config?.name === "Color" ? (
            <>
              <label style={{ textTransform: "uppercase " }}>
                {t(`createNewLook.${config.name.toLowerCase()}`)}
              </label>
              <br />
              <Checkbox.Group
                onChange={(values) =>
                  ...
                  config?.items?.map((item) => (
                    <ButtonWrapper
                      ...
                      onClick={() =>
                        setFilterData((filter) => ({
                          ...filter,
                          [config.source]: xor(get(filter, config.source, []), [
                            item._id,
                          ]),
                        }))
                      }
